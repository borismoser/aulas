package av3.correcao.marlon.Connector;
...
import av3.correcao.marlon.Model.Cotacao;
...
public class Banco {
	...
	private static ArrayList<Cotacao> lista = new ArrayList();
	...
	public static void b() {
		String sql = "SELECT A.*, A.fechamento-A.abertura as A.dif FROM cotacao A ORDER BY data DESC";
		try (Statement st = conexao().createStatement()) {
			ResultSet rs = st.executeQuery(sql);
			System.out.println("Símbolo |    Data    |   Abertura  |  Fechamento  | Volume");
			while (rs.next()) {
				System.out.println(rs.getString("simbolo") + "   | " + formato.format(rs.getDate("data")) + " |    " + rs.getDouble("abertura")
						+ "    |    " + rs.getDouble("fechamento") + "      | " + rs.getInt("volume") + " | " + rs.getDouble("dif"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
	...
	public static void c() {
		String sql = "SELECT A.*, A.fechamento-A.abertura as A.dif FROM cotacao A";
		try (Statement st = conexao().createStatement()) {
			ResultSet rs = st.executeQuery(sql);
			while (rs.next()) {
				lista.add(new Cotacao(rs.getString("simbolo"), rs.getDate("data"), rs.getDouble("abertura"), rs.getDouble("fechamento"), rs.getInt("volume"), rs.getDouble("dif")));
			}
			Cotacao bigABEV3 = new Cotacao();
			Cotacao bigITUB4 = new Cotacao();
			Cotacao bigPETR4 = new Cotacao();
			Cotacao bigVALE3 = new Cotacao();
			for (Cotacao x : lista) {
				if (x.getSimbolo().equals("ABEV3")) {
					if (Objects.isNull(bigABEV3.getDif()) || x.getDif() > bigABEV3.getDif()) {
						bigABEV3 = x;
					}
				} else if (x.getSimbolo().equals("ITUB4")) {
					if (Objects.isNull(bigITUB4.getDif()) || x.getDif() > bigITUB4.getDif()) {
						bigITUB4 = x;
					}
				} else if (x.getSimbolo().equals("PETR4")) {
					if (Objects.isNull(bigPETR4.getDif()) || x.getDif() > bigPETR4.getDif()) {
						bigPETR4 = x;
					}
				} else if (x.getSimbolo().equals("VALE3")) {
					if (Objects.isNull(bigVALE3.getDif()) || x.getDif() > bigVALE3.getDif()) {
						bigVALE3 = x;
					}
				}
			}
			System.out.println(bigABEV3.toString());
			System.out.println(bigITUB4.toString());
			System.out.println(bigPETR4.toString());
			System.out.println(bigVALE3.toString());
		} catch (SQLException e) {
			e.printStackTrace();
		}
	}
	
	public static void d(String data, Double valor) {
		for (Cotacao x : lista) {
			if (formato.format(x.getData()).equals(data)) {
				double d = valor / x.getAbertura();
				int resultado = (int) d;
				System.out.println(x.toString());
				System.out.println("Possível compra: "+resultado);
			}
		}
	}
